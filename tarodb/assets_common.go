...
func upsertAssetsWithGenesis(ctx context.Context, q UpsertAssetStore,
	...
	for idx, a := range assets {
		...
		//   * inserting the fam key rn, which is ok as its external w/
		//     no key desc info
		var familySigID sql.NullInt32
		if a.FamilyKey != nil {
			// Before we can insert a new asset key family, we'll
			// also need to insert an internal key which will be
			// referenced by the key family.
			...
			if err != nil {
				return 0, nil, fmt.Errorf("unable to insert "+
					"internal key: %w", err)
			}
			famKeyBytes := familyKey.FamKey.SerializeCompressed()
			assetKey := AssetFamilyKey{
				TweakedFamKey:  famKeyBytes,
				InternalKeyID:  keyID,
				GenesisPointID: genesisPointID,
			}
			...
			// With the statement above complete, we'll now insert
			// the asset_family_sig entry for this, which has a
			// one-to-many relationship with family keys (there can
			// be many sigs for a family key which link together
			// otherwise disparate asset IDs).
			famSigID, err := q.InsertAssetFamilySig(
				ctx, AssetFamSig{
		...
		if err != nil {
			return 0, nil, fmt.Errorf("unable to upsert "+
				"script key: %w", err)
		}
